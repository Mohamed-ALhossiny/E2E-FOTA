...
#define OS_I64                               long long           // Compiler supports 64-bit data type
...
#define OS_ENABLE_INTS_SAVE_IPL()                                // Not required with Cortex-M, avoid call of OS_INT_Enable()
  ...
  #define OS_IPL_THRESHOLD                   128                 // Can be overwritten in OS_Config.h or as preprocessor setting
#endif
...
#define OS_INT_EnableAll()       { __asm volatile ("cpsie i" : : : "memory"); }
  ...
  #define OS_INT_Preserve(p)     OS_INT_PreserveAll(p)
  #define OS_INT_Restore(p)      OS_INT_RestoreAll(p)
  //
  // OS_INT_PreserveAll() / OS_INT_RestoreAll()
...
extern const struct OS_EXTEND_TASK_CONTEXT_STRUCT OS_TLS_ContextExtension;
void OS_TLS_Set                    (struct _reent * pReentStruct) OS_TEXT_SECTION_ATTRIBUTE(OS_TLS_Set);
void OS_TLS_SetTaskContextExtension(struct _reent * pReentStruct) OS_TEXT_SECTION_ATTRIBUTE(OS_TLS_SetTaskContextExtension);
//
// Macros for compatibility with older embOS version
//
#define OS_ExtendTaskContext_TLS        OS_TLS_SetTaskContextExtension
#define OS_InitReent                    OS_TLS_SetTaskContextExtension
...
void OS_PSPLIM_Set                    (const void* pStack) OS_TEXT_SECTION_ATTRIBUTE(OS_PSPLIM_Set);
void OS_PSPLIM_SetTaskContextExtension(const void* pStack) OS_TEXT_SECTION_ATTRIBUTE(OS_PSPLIM_SetTaskContextExtension);

/*********************************************************************
*
*       Support for Armv8-M TrustZone
*
*       This support assumes embOS is running in non secure state but
*       is able to call secure functions from tasks.
*/
typedef struct OS_ARM_TZ_SECURE_API_LIST_STRUCT {
  unsigned long (*GetCONTROL_s)(void);
  unsigned long (*GetPSP_s)    (void);
  unsigned long (*GetPSPLIM_s) (void);
  void          (*SetCONTROL_s)(unsigned long CONTROL_s);
  void          (*SetPSP_s)    (unsigned long PSP_s);
  void          (*SetPSPLIM_s) (unsigned long PSPLIM_s);
} OS_ARM_TZ_SECURE_API_LIST;

extern const struct OS_EXTEND_TASK_CONTEXT_STRUCT OS_ARM_TZ_ContextExtension;
void OS_ARM_TZ_SetSecureStatePSP      (OS_ARM_TZ_SECURE_API_LIST* ApiList, const void* pStack, unsigned long StackSize) OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_TZ_SetSecureStatePSP);
void OS_ARM_TZ_SetTaskContextExtension(OS_ARM_TZ_SECURE_API_LIST* ApiList, const void* pStack, unsigned long StackSize) OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_TZ_SetTaskContextExtension);
...
*/
  ...
  #define OS_SUPPORT_TRACKNAME_DEFAULT         (1)
  #define OS_SUPPORT_SAVE_RESTORE_HOOK_DEFAULT (1)
  #define OS_TASK_Create                       OS_TASK_Create
  #define OS_TASK_CreateEx                     OS_TASK_CreateEx
  #define OS_LIBMODE                           "D"
  #define OS_SUPPORT_STAT_DEFAULT              (1)
  #define OS_INIT_EXPLICITLY_DEFAULT           (0)
...
struct OS_TASK_STRUCT {
  ...
  OS_PRIO                     Priority;         // Task priority, could be raised during Mutex handling
  OS_PRIO                     BasePrio;         // Initial task priority
...
**********************************************************************
*/
void    OS_MAILBOX_Clear           (OS_MAILBOX* pMB)                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Clear);
...
char    OS_MAILBOX_Get             (OS_MAILBOX* pMB, void* pDest)                                       OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Get);
char    OS_MAILBOX_Get1            (OS_MAILBOX* pMB, char* pDest)                                       OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Get1);
...
char    OS_MAILBOX_Peek            (OS_CONST_PTR OS_MAILBOX* pMB, void* pDest)                          OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Peek);
void    OS_MAILBOX_Purge           (OS_MAILBOX* pMB)                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Purge);
char    OS_MAILBOX_Put             (OS_MAILBOX* pMB, OS_CONST_PTR void* pMail)                          OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Put);
char    OS_MAILBOX_Put1            (OS_MAILBOX* pMB, OS_CONST_PTR char* pMail)                          OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_Put1);
void    OS_MAILBOX_PutBlocked      (OS_MAILBOX* pMB, OS_CONST_PTR void* pMail)                          OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutBlocked);
void    OS_MAILBOX_PutBlocked1     (OS_MAILBOX* pMB, OS_CONST_PTR char* pMail)                          OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutBlocked1);
char    OS_MAILBOX_PutFront        (OS_MAILBOX* pMB, OS_CONST_PTR void* pMail)                          OS_TEXT_SECTION_ATTRIBUTE(OS_MAILBOX_PutFront);
...
void OS_INT_CallNestable(void (*pfRoutine)(void)) OS_TEXT_SECTION_ATTRIBUTE(OS_INT_CallNestable);
  ...
  //
  // Required for CPUs which do not restore DI-flag by return from interrupt instruction. Currently used in embOS Cortex-M only.
  //
