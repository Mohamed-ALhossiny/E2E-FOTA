/**
  *******************************************************************************
  * @file    legacy_v3_hmac_sha256.c
  * @author  MCD Application Team
  * @brief   This file provides HMAC SHA256 functions of helper for
...
int32_t HMAC_SHA256_Init(HMAC_SHA256ctx_stt *P_pHMAC_SHA256ctx)
{
  ...
  if (P_pHMAC_SHA256ctx == NULL)
  {
    retval = HASH_ERR_BAD_PARAMETER;
    goto error;
  }
  P_pHMAC_SHA256ctx->mac_handle = cmox_hmac_construct(&P_pHMAC_SHA256ctx->hmac_handle,
                                                      CMOX_HMAC_SHA256);
  if (P_pHMAC_SHA256ctx->mac_handle == NULL)
  {
    retval = HASH_ERR_BAD_PARAMETER;
    goto error;
  }
  cmox_retval = cmox_mac_init(P_pHMAC_SHA256ctx->mac_handle);
  ...
  cmox_retval = cmox_mac_setTagLen(P_pHMAC_SHA256ctx->mac_handle, (size_t)P_pHMAC_SHA256ctx->mTagSize);
  if (cmox_retval != CMOX_MAC_SUCCESS)
  {
  ...
  cmox_retval = cmox_mac_setKey(P_pHMAC_SHA256ctx->mac_handle,
                                P_pHMAC_SHA256ctx->pmKey,
                                (size_t)P_pHMAC_SHA256ctx->mKeySize);
  if (cmox_retval != CMOX_MAC_SUCCESS)
  {
    retval = HASH_ERR_BAD_CONTEXT;
    goto error;
...
/* HMAC-SHA256 Update function, process input data and update a HMAC_SHA256ctx_stt */
int32_t HMAC_SHA256_Append(HMAC_SHA256ctx_stt *P_pHMAC_SHA256ctx, const uint8_t *P_pInputBuffer, int32_t P_inputSize)
{
  cmox_mac_retval_t cmox_retval;
  int32_t retval;

  if ((P_pHMAC_SHA256ctx == NULL) || (P_pInputBuffer == NULL) || (P_inputSize < 0))
  {
    retval = HASH_ERR_BAD_PARAMETER;
    goto error;
  }

  cmox_retval = cmox_mac_append(P_pHMAC_SHA256ctx->mac_handle, P_pInputBuffer, (size_t)P_inputSize);
...
/* HMAC-SHA256 Finish function, produce the output HMAC-SHA256 tag */
int32_t HMAC_SHA256_Finish(HMAC_SHA256ctx_stt *P_pHMAC_SHA256ctx, uint8_t *P_pOutputBuffer, int32_t *P_pOutputSize)
{
  ...
  if ((P_pHMAC_SHA256ctx == NULL) || (P_pOutputBuffer == NULL) || (P_pOutputSize == NULL))
  {
    retval = HASH_ERR_BAD_PARAMETER;
    goto error;
  }

  cmox_retval = cmox_mac_generateTag(P_pHMAC_SHA256ctx->mac_handle, P_pOutputBuffer, &tempOutputLen);
  ...
  *P_pOutputSize = (int32_t)tempOutputLen;
  ...
  cmox_retval = cmox_mac_cleanup(P_pHMAC_SHA256ctx->mac_handle);
  if (cmox_retval != CMOX_MAC_SUCCESS)
  {
    retval = HASH_ERR_BAD_PARAMETER;
    goto error;
