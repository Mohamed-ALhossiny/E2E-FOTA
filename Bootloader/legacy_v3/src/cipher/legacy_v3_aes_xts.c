/**
  *******************************************************************************
  * @file    legacy_v3_aes_xts.c
  * @author  MCD Application Team
  * @brief   This file provides AES XTS functions of helper for migration
...
/* Initialization for AES Encryption in XTS Mode */
int32_t AES_XTS_Encrypt_Init(AESXTSctx_stt *P_pAESXTSctx, const uint8_t *P_pKey, const uint8_t *P_pIv)
{
  ...
  if (P_pAESXTSctx == NULL)
  {
    retval = AES_ERR_BAD_PARAMETER;
    goto error;
  }

  P_pAESXTSctx->cipher_handle = cmox_xts_construct(&P_pAESXTSctx->xts_handle, CMOX_AESFAST_XTS_ENC);
  if (P_pAESXTSctx->cipher_handle == NULL)
  {
    retval = AES_ERR_BAD_PARAMETER;
    goto error;
  }

  cmox_retval = cmox_cipher_init(P_pAESXTSctx->cipher_handle);
  if (cmox_retval != CMOX_CIPHER_SUCCESS)
  {
    retval = AES_ERR_BAD_PARAMETER;
    goto error;
  }
  cmox_retval = cmox_cipher_setKey(P_pAESXTSctx->cipher_handle, P_pKey, (size_t)P_pAESXTSctx->mKeySize);
  if (cmox_retval != CMOX_CIPHER_SUCCESS)
  {
    retval = AES_ERR_BAD_PARAMETER;
    goto error;
  }
  cmox_retval = cmox_cipher_setIV(P_pAESXTSctx->cipher_handle, P_pIv, (size_t)P_pAESXTSctx->mIvSize);
  if (cmox_retval != CMOX_CIPHER_SUCCESS)
  {
    retval = AES_ERR_BAD_PARAMETER;
    goto error;
                               ...
                               int32_t       *P_pOutputSize)
{
  ...
  cmox_retval = cmox_cipher_append(P_pAESXTSctx->cipher_handle, P_pInputBuffer, (size_t)P_inputSize,
...
/* AES Encryption Finalization of XTS Mode */
                               ...
                               int32_t       *P_pOutputSize)
{
  ...
  int32_t retval;
  /* Parameter checking */
  if ((P_pAESXTSctx == NULL) || (P_pOutputSize == NULL))
  {
    retval = AES_ERR_BAD_PARAMETER;
    goto error;
  }

  P_pAESXTSctx->mFlags |= E_SK_OPERATION_COMPLETED;
  *P_pOutputSize = 0;
  retval = AES_SUCCESS;
...
/* Initialization for AES Decryption in XTS Mode */
int32_t AES_XTS_Decrypt_Init(AESXTSctx_stt *P_pAESXTSctx, const uint8_t *P_pKey, const uint8_t *P_pIv)
{
  ...
  P_pAESXTSctx->cipher_handle = cmox_xts_construct(&P_pAESXTSctx->xts_handle, CMOX_AESFAST_XTS_DEC);
  if (P_pAESXTSctx->cipher_handle == NULL)
  {
    retval = AES_ERR_BAD_PARAMETER;
    goto error;
...
/* AES Decryption Finalization of XTS Mode */
