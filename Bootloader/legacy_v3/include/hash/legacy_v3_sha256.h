...
#ifndef LEGACY_V3_SHA256_H
#define LEGACY_V3_SHA256_H

#include <STD_TYPE.h>
#include "../../Core/Inc/include/hash/cmox_sha256.h"

#include "legacy_v3_hash.h"
...
#define CRL_SHA256_SIZE CMOX_SHA256_SIZE   /*!< Number of bytes (uint8_t) to store a SHA-256 digest. */
...
typedef struct
{
  cmox_sha256_handle_t sha_handle;    /*!< CMOX SHA256 handle */
...
/**
  ...
  * @param[in,out]  *P_pSHA256ctx The context that will be initialized
  * @retval HASH_SUCCESS Operation Successful
  * @retval HASH_ERR_BAD_PARAMETER Parameter P_pSHA256ctx is invalid
  * @note \c P_pSHA256ctx.mFlags must be set prior to calling this function. Default value is E_HASH_DEFAULT.
  *          See \ref HashFlags_et for details.
  * @note \c P_pSHA256ctx.mTagSize must be set with the size of the required message digest that will be generated
  *       by the \ref SHA256_Finish
  *          Possible values are values are from 1 to CRL_SHA256_SIZE (32)
  */
int32_t SHA256_Init(SHA256ctx_stt *P_pSHA256ctx);

/**
  * @brief  SHA256 Update function, process input data and update a SHA256ctx_stt
  * @param[in,out] *P_pSHA256ctx      The SHA256 context that will be updated
  ...
  * @retval HASH_ERR_BAD_OPERATION  SHA256_Append can't be called after \ref SHA256_Finish has been called.
...
/**
  * @brief  SHA256 Finish function, produce the output SHA256 digest
  * @param[in,out] *P_pSHA256ctx        The SHA256 context
  ...
  * @note   P_pSHA256ctx->mTagSize must contain a valid value, between 1 and CRL_SHA256_SIZE (32)
